...
Консольное приложение управляет массивом записей `Lesson` (до 1000 элементов) и строит индексы для сортировки/поиска по любому полю. Используются только сырые массивы и простые функции (никаких STL-контейнеров), строки интерфейса вынесены в `messages.cpp`.
...
- Меню позволяет выбрать поле индекса (название/преподаватель/дата+время/аудитория/тип/приоритет), построить индекс, вывести по возрастанию/убыванию или выполнить поиск по текущему индексу. Все совпадения по ключу выводятся.
...
- Никаких пользовательских пространств имён и контейнеров STL; только `std::string` и потоки ввода/вывода.
- Строковые индексы балансируются AVL-поворотами, числовые — простой вставкой с последующей сортировкой вставкой.
